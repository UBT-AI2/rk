...
void VERNER65(double ***A, double **b, double **b_hat, double **c, int *s,
              int *ord)
{
  /*
   * Source: J. H. Verner. Explicit Runge-Kutta Methods with Estimates of the
   * Local Truncation Error. SIAM J. Numer. Anal., 15(4), 772–790, Aug. 1978,
   * DOI 10.1137/0715051, Table 5
   */
  ...
  *ord = 6;
  ...
  {
#endif
    printf("Method: VERNER65 (order %d, %d stages)\n", *ord, *s);
#ifdef HAVE_MPI
  }
#endif
  ...
  (*b_hat)[1] = 0.0;
  (*b_hat)[2] = -16.0 / 65.0;
  (*b_hat)[3] = 1377.0 / 2240.0;
  (*b_hat)[4] = 121.0 / 320.0;
  (*b_hat)[5] = 0.0;
  (*b_hat)[6] = 891.0 / 8320.0;
  (*b_hat)[7] = 2.0 / 35.0;
  ...
  /* Just to leave nothing uninitialized... */
  for (i = 0; i < *s; i++)
    for (j = i; j < *s; j++)
      (*A)[i][j] = 0.0;
}
