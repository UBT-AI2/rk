...
void *solver_thread(void *argument)
{
  ...
  FOR_ALL_GRIDPOINTS(t0, te, h, steps_acc, steps_rej)
  {
    ...
    for (j = 1; j < s; j++)
    {
      ...
      for (i = 1; i < j; i++)
        block_interm_stage(i, first + (2 * j - 1) * BLOCKSIZE - i * BLOCKSIZE,
                           BLOCKSIZE, s, t, h, A, b, b_hat, c, y, err, dy, w);

      block_first_stage(first + 2 * j * BLOCKSIZE, BLOCKSIZE,
                        s, t, h, A, b, b_hat, c, y, err, dy, w);
      for (i = 1; i < j; i++)
        block_interm_stage(i, first + 2 * j * BLOCKSIZE - i * BLOCKSIZE,
                           BLOCKSIZE, s, t, h, A, b, b_hat, c, y, err, dy, w);
    }

    /* sweep */

    for (j = first + (2 * s - 1) * BLOCKSIZE; j < (last - BLOCKSIZE + 1);
         j += BLOCKSIZE)
    {
  ...
  finalize_left:
    ...
    for (i = 1; i < s - 1; i++)
      block_interm_stage(i, first + i * BLOCKSIZE, BLOCKSIZE,
                         s, t, h, A, b, b_hat, c, y, err, dy, w);

    block_last_stage(first + (s-1) * BLOCKSIZE, BLOCKSIZE,
		     s, t, h, b, b_hat, c, y, err, dy, w, &err_max);

    for (j = 1; j < s - 1; j++)
    {
      wait_for_pred(me, j, mutex_last);
      block_interm_stage(j, first, BLOCKSIZE,
                         s, t, h, A, b, b_hat, c, y, err, dy, w);
      release_pred(me, j, mutex_last);
      first_block_complete(me, j + 1, mutex_first);

      for (i = j + 1; i < s - 1; i++)
        block_interm_stage(i, first + (i - j) * BLOCKSIZE, BLOCKSIZE,
                           s, t, h, A, b, b_hat, c, y, err, dy, w);

      block_last_stage(first + (s - 1 - j) * BLOCKSIZE, BLOCKSIZE,
                       s, t, h, b, b_hat, c, y, err, dy, w, &err_max);
    }

    wait_for_pred(me, s - 1, mutex_last);
    block_last_stage(first, BLOCKSIZE,
                     s, t, h, b, b_hat, c, y, err, dy, w, &err_max);
  ...
  finalize_right:
    ...
    for (j = 1; j < s - 1; j++)
    {
      ...
      block_interm_stage(j, (last - BLOCKSIZE + 1), BLOCKSIZE,
                         s, t, h, A, b, b_hat, c, y, err, dy, w);
      release_succ(me, j, mutex_first);
      last_block_complete(me, j + 1, mutex_last);

      for (i = j + 1; i < s - 1; i++)
        block_interm_stage(i, (last - BLOCKSIZE + 1) - (i - j) * BLOCKSIZE,
                           BLOCKSIZE, s, t, h, A, b, b_hat, c, y, err, dy, w);

      block_last_stage((last - BLOCKSIZE + 1) - (s - 1 - j) * BLOCKSIZE,
                       BLOCKSIZE, s, t, h, b, b_hat, c, y, err, dy, w,
                       &err_max);
    }

    wait_for_succ(me, s - 1, mutex_first);
    block_last_stage((last - BLOCKSIZE + 1), BLOCKSIZE,
                     s, t, h, b, b_hat, c, y, err, dy, w, &err_max);
...
void solver(double t0, double te, double *y0, double *y, double tol)
{
  ...
  printf("(pipelining with blockwise neighbor-to-neighbor communication)\n");
